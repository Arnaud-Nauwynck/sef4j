...
public class EventLoggerFactory {
	...
	protected Map<String,EventSender[]> safeGetInheritedAppenderFor(Set<String> eventLoggerNames) {
		Map<String,EventSender[]> res;
		try {
			res = eventLoggerContext.getInheritedAppendersFor(eventLoggerNames);		
		} catch(Exception ex) {
			// should not occurs... but fallback to safe loop
			res = new HashMap<String,EventSender[]>();
	...
	private void doReconfigureAppenderForLoggers(Map<String,EventLogger> loggers) {
		Map<String,EventSender[]> appendersPerLogger = safeGetInheritedAppenderFor(loggers.keySet());
		for(EventLogger eventLogger : loggers.values()) {
			String eventLoggerName = eventLogger.getEventLoggerName();
			EventSender[] appenders = appendersPerLogger.get(eventLoggerName);
			if (appenders == null) {
				// should not occur
