...
import org.sef4j.slf4jbridge.utils.Slf4jLoggerUtils;
 ...
 * 
 * ... to be extended for enriching poor logback with MDC, or
 * ElasticSearch-logback appender with richer document structure
 * 
 * sample usages: <code>
 ...
 *
 *
 * Notice that when using EventLoggerAdapterAppender the flat formatted message
 * will be used for all appenders except param-aware appender class
 * EventLoggerAdapterAppender For EventLoggerAdapterAppender class, the rich
 * event log event will use <code>Map&lt;String,Object> values</code> with
 * params filled from LoggerExt info() arguments and overriden local stack
 * params from LocalCallStack
 * 
 * Internally, message will be temporary masked for EventLoggerAdapterAppender,
 * then unmasked and replaced by rich name-param specific for
 * EventLoggerAdapterAppender see schema explanation below
 * 
 * 
 * 
 * <PRE>
 ...
 * 
 * 
 * Remarks using slf4j Marker class: we could have used slf4j Marker interface,
 * by providing a specific NameValuesMarker sub class (or registering a
 * NameValueMarker into SimpleMarker list) but it seems not widely used by
 * knwown implementation of slf4j ... and would interfer with other markers?!
 * for a such implementation usage, see LogbackLogstash project, class :
 * net.logstash.logback.marker.Marker their implementation gives the following
 * code style:
 * 
 * <pre>
 * {@code
...
public class LoggerExt {
    ...
    private static LoggerExtFactory defaultFactory = new LoggerExtFactory(
	    LoggerFactory.getILoggerFactory(),
	    new Slf4jToLoggingEventExtMapper());
    
    public static LoggerExtFactory getDefaultFactory() {
        return defaultFactory;
    }

    public static void setDefaultFactory(LoggerExtFactory defaultFactory) {
        LoggerExt.defaultFactory = defaultFactory;
    }

    public static LoggerExt getLogger(String name) {
	return defaultFactory.create(name);
    }

    public static LoggerExt getLogger(Class<?> clss) {
	return defaultFactory.create(clss.getName());
    }

    public static LoggerExt wrap(Logger logger) {
	return defaultFactory.wrap(logger);
    }
    ...
    public LoggerExt(Logger slf4jLogger, Slf4jToLoggingEventExtMapper slf4jToEventExtMapper) {
    ...
    public void traceNV(String msgFragment0, Object value0, String msgFragment1, Object value1, String msgFragment2,
	    Object value2) {
	MsgBuilder b = new MsgBuilder();
	appendMsgFragmentValue(b, 0, msgFragment0, value0);
	appendMsgFragmentValue(b, 1, msgFragment1, value1);
    ...
    public void traceNV(String msgFragment0, Object value0, String msgFragment1, Object value1, String msgFragment2,
	    Object value2, String msgFragment3, Object value3) {
	MsgBuilder b = new MsgBuilder();
	appendMsgFragmentValue(b, 0, msgFragment0, value0);
	appendMsgFragmentValue(b, 1, msgFragment1, value1);
	appendMsgFragmentValue(b, 2, msgFragment2, value2);
    ...
    public void traceNV(String msgFragment0, Object value0, String msgFragment1, Object value1, String msgFragment2,
	    Object value2, String msgFragment3, Object value3, Object... args) {
	MsgBuilder b = new MsgBuilder();
	appendMsgFragmentValue(b, 0, msgFragment0, value0);
	appendMsgFragmentValue(b, 1, msgFragment1, value1);
	appendMsgFragmentValue(b, 2, msgFragment2, value2);
    ...
    public void debugNV(String msgFragment0, Object value0, String msgFragment1, Object value1, String msgFragment2,
	    Object value2) {
	MsgBuilder b = new MsgBuilder();
	appendMsgFragmentValue(b, 0, msgFragment0, value0);
	appendMsgFragmentValue(b, 1, msgFragment1, value1);
    ...
    public void infoNV(String msgFragment0, Object value0, String msgFragment1, Object value1, String msgFragment2,
	    Object value2) {
	MsgBuilder b = new MsgBuilder();
	appendMsgFragmentValue(b, 0, msgFragment0, value0);
	appendMsgFragmentValue(b, 1, msgFragment1, value1);
    ...
    public void warnNV(String msgFragment0, Object value0, String msgFragment1, Object value1, String msgFragment2,
	    Object value2) {
	MsgBuilder b = new MsgBuilder();
	appendMsgFragmentValue(b, 0, msgFragment0, value0);
	appendMsgFragmentValue(b, 1, msgFragment1, value1);
    ...
    public void errorNV(String msgFragment0, Object value0, String msgFragment1, Object value1, String msgFragment2,
	    Object value2) {
	MsgBuilder b = new MsgBuilder();
	appendMsgFragmentValue(b, 0, msgFragment0, value0);
	appendMsgFragmentValue(b, 1, msgFragment1, value1);
	...
	return Slf4jLoggerUtils.isEnabled(slf4jLogger, logLevel);
    }
	...
	if (values == null || values.isEmpty()) {
	    Slf4jLoggerUtils.logLevelText(slf4jLogger, logLevel, text);
	} else {
	    LoggingEventExt event = buildLoggingEventExt(logLevel, text, templateText, values, null);
	    Slf4jAppenderThreadLocalMask.logWithEnrichedLogEventExt(
		    event, () -> Slf4jLoggerUtils.logLevelText(slf4jLogger, logLevel, text));
	}
    }
	...
	if (values == null || values.isEmpty()) {
	    Slf4jLoggerUtils.logLevelTextException(slf4jLogger, logLevel, text, ex);
	} else {
	    LoggingEventExt event = buildLoggingEventExt(logLevel, text, templateText, values, ex);
	    Slf4jAppenderThreadLocalMask.logWithEnrichedLogEventExt(
		    event, () -> Slf4jLoggerUtils.logLevelTextException(slf4jLogger, logLevel, text, ex));
	}
    }

    protected LoggingEventExt buildLoggingEventExt(LogLevel logLevel, String text, String templateText,
	    Map<String, Object> values, Throwable ex) {
	return slf4jToEventExtMapper.buildEvent(slf4jLogger.getName(), logLevel, text, templateText, true, true,
		true, // <= fillCallStackPath, fillInheritedProps, fillParams
		values, ex);
    }

    protected static void appendMsgFragmentValue(MsgBuilder b, int paramIndex, String msgFragment, Object value) {
	...
	if (paramName != null) {
		...
		// append value in formatted text, and append implicit "{{name}}" in
		// templateText
	    ...
	    // error: bad formatted msgFragment, should ends with "{{name}}", "name:" or
	    // "name="
