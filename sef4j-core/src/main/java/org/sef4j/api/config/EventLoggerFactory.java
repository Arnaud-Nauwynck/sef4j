...
import org.sef4j.api.config.EventLoggerContext.EventLoggerContextListener;
 ...
 * 
 * This class is a factory and the container manager for all created EventLogger
 * elements. This class uses EventLoggerContext to determine which EventSenders
 * to attach to EventLoggers. The EventContext may be changed at runtime for a
 * EventLoggerFactory
 * 
 * it is preferable to use this class and EventLogger injected by an IOC
 * container ... But you can also use default static instance, store in static
 * singleton EventLoggerFactoryStaticBinder
 * 
 */
public class EventLoggerFactory {
    ...
    private static final EventAppender<Object>[] EMPTY_EVENT_SENDER_ARRAY = (EventAppender<Object>[]) new EventAppender<?>[0];
	...
	if (res == null) {
	    EventAppender<Object>[] inheritedAppenders = safeGetInheritedAppenderFor(eventLoggerName);
	    synchronized (lock) {
		res = eventLoggers.get(eventLoggerName);
		if (res == null) {
    ...
    protected EventAppender<Object>[] safeGetInheritedAppenderFor(String eventLoggerName) {
	...
	try {
	    ...
	    LOG.error("Failed to compute inherited appenders for " + eventLoggerName + "! .. use empty, no rethrow ex",
		    ex);
	    // ignore, no rethrow!
	}
    ...
    protected Map<String, EventAppender<Object>[]> safeGetInheritedAppenderFor(Set<String> eventLoggerNames) {
	Map<String, EventAppender<Object>[]> res;
	try {
	    res = eventLoggerContext.getInheritedAppendersFor(eventLoggerNames);
	} catch (Exception ex) {
	    // should not occurs... but fallback to safe loop
	    res = new HashMap<String, EventAppender<Object>[]>();
	...
	synchronized (lock) {
		...
		if (name.startsWith(parentEventLoggerName) && (name.length() == parentEventLoggerName.length()
	...
	Map<String, EventAppender<Object>[]> appendersPerLogger = safeGetInheritedAppenderFor(loggers.keySet());
	for (EventLogger eventLogger : loggers.values()) {
	    String eventLoggerName = eventLogger.getEventLoggerName();
	    EventAppender<Object>[] appenders = appendersPerLogger.get(eventLoggerName);
	    if (appenders == null) {
		// should not occur
