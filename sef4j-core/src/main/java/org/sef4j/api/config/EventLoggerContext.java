...
public class EventLoggerContext {
    ...
    private Map<String, EventAppender<Object>> appenders = new HashMap<String, EventAppender<Object>>();
	...
	rootLoggerCtx.inheritedAppenders = (EventAppender<Object>[]) new EventAppender<?>[0];
    ...
    @SuppressWarnings("unchecked")
    public <T> void addAppender(String appenderName, EventAppender<T> eventSender) {
	synchronized (lock) {
	    if (appenders.get(appenderName) != null) {
		throw new IllegalArgumentException("appender name '" + appenderName + "' already used");
	    }
	    ;
	    appenders.put(appenderName, (EventAppender<Object>) eventSender);
	...
	synchronized (lock) {
	    EventAppender<Object> appender = appenders.remove(appenderName);
	    if (appender == null) {
	    ...
	    Map<String, PerLoggerContext> loggerToReeval = new TreeMap<String, PerLoggerContext>(); // in sorted order:
												    // "a" before "a.b"
												    // ...
    ...
    public void addLoggerToAppenderRef(String eventLoggerName, String appenderName, boolean addOrRemoveInheritable
    // TOADD: EventFilter...
    ) {
	synchronized (lock) {
	    EventAppender<Object> appender = this.appenders.get(appenderName);
	    if (appender == null) {
    ...
    @SuppressWarnings("unchecked")
    public <E> EventAppender<E>[] getInheritedAppendersFor(String eventLoggerName) {
	synchronized (lock) {
	    PerLoggerContext loggerCxt = findFirstAncestorLoggerContextFor(eventLoggerName);
	    return (EventAppender<E>[]) loggerCxt.inheritedAppenders;
	}
    }

    @SuppressWarnings("unchecked")
    public <E> Map<String, EventAppender<E>[]> getInheritedAppendersFor(Set<String> eventLoggerNames) {
	Map<String, EventAppender<E>[]> res = new HashMap<String, EventAppender<E>[]>();
	synchronized (lock) {
	    for (String eventLoggerName : eventLoggerNames) {
		PerLoggerContext loggerCxt = findFirstAncestorLoggerContextFor(eventLoggerName);
		res.put(eventLoggerName, (EventAppender<E>[]) loggerCxt.inheritedAppenders);
	    }
	}
	return res;
    }
	...
	List<EventAppender<Object>> tmpAppenders = new ArrayList<EventAppender<Object>>();
	...
	EventAppender<Object>[] newInheritedAppenders = (EventAppender<Object>[]) tmpAppenders
		.toArray(new EventAppender<?>[tmpAppenders.size()]);
    ...
    private void collectAddOrRemoveAppenders(List<EventAppender<Object>> resAppenders,
	    PerLoggerContext eventLoggerConf) {
	for (AppenderRefContext l2a : eventLoggerConf.toAppenderRefs.values()) {
	    String appenderName = l2a.getAppenderName();
	    EventAppender<Object> appender = appenders.get(appenderName);
