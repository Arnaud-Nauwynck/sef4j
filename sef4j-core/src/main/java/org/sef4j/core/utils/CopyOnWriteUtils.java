...
public class CopyOnWriteUtils {
	...
	return new ImmutableMap.Builder<K, V>().putAll(src).put(key, value).build();
    }
    ...
    // native Array copy-on-write utilities (... need extra Class "componentType"
    // argument for handling empty array from generic class ctors!!)
    // ------------------------------------------------------------------------
