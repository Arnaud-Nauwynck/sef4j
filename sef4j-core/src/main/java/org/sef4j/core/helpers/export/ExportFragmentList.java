...
public class ExportFragmentList<T> implements ExportFragmentsAdder<T> {
	...
	public List<ExportFragment<T>> toList() {
		List<ExportFragment<T>> res = new ArrayList<ExportFragment<T>>();
		copyTo(res);
		return res;
	}
	...
	public Map<Object,T> identifiableFragmentsToValuesMap() {
		Map<Object,T> res = new HashMap<Object,T>();
		for(ExportFragment<T> e : identifiableFragments.values()) {
			res.put(e.getId(), e.getValue());
		}
		return res;
	}
