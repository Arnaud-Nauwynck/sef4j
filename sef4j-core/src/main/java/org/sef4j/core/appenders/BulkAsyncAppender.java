 ...
 * 
 * This class bufferize Events in memory, and send bulk events, either after a
 * wait delay (example: 15 secondes), or after the bulk is "filled enough":
 * having reached maxEventsCount or having reached a cumulated of
 * maxBulkByteLength
 *
 * <PRE>
 *   Unitary          +-------------------+    _
 ...
 * </PRE>
 */
public class BulkAsyncAppender<T> implements EventAppender<T> {

    private static final Logger LOG = LoggerFactory.getLogger(BulkAsyncAppender.class);
    private static final boolean DEBUG = true;

    protected EventAppender<T> targetOutput;
    ...
    // this "temporary" queue is used to send bulk events asynchronously using
    // another thread
    ...
    private BulkAsyncAppender(EventAppender<T> target, Builder<T> builder) {
	...
	synchronized (lock) {
		...
		if ((bufferedEvents.size() + 1 > maxBulkEventsCount)
		    ...
		    if (DEBUG) {
			if (bufferedEvents.size() + 1 > maxBulkEventsCount) {
			    LOG.info(System.currentTimeMillis() + " sendEvent .. reached maxBulkEventsCount "
				    + maxBulkEventsCount + " .. need flush");
			} else {
			    LOG.info(System.currentTimeMillis() + " sendEvent .. reached maxBulkByteLength:"
				    + bufferedeBulkByteLength + " + " + eventByteLen + " .. need flush");
			}
		    }
		    ...
		    needFlush = !asyncEventsBulksQueue.isEmpty(); // mostly true ... cf disruptorErrorHandler to purge
								  // queue
		}
		...
		if (!flushFilledBulkInCurrentThread) {
		    // do not flush in current thread (and inside lock) => schedule immediate in
		    // separate thread
		    scheduleFlush(0);
		} else {
	...
	if (DEBUG)
	    LOG.info(System.currentTimeMillis() + " flush " + flushParamToString(forceSendAll, sendPartiallyFilledBulk)
		    + " " + infoToString());
	...
	synchronized (lock) {
	    if (DEBUG)
		LOG.info(System.currentTimeMillis() + " scheduleFlush " + delay);
	    long nextTime = System.currentTimeMillis() + delay * 1000;
	    if (scheduledFuture != null && scheduledFutureTime > nextTime + 400) { // 400 ms: precision for not
										   // cancel+reschedule!
	...
	if (DEBUG)
	    LOG.info(System.currentTimeMillis() + " cancel schedule " + scheduledFutureTime);
	scheduledFuture.cancel(false);
	scheduledFuture = null;
	scheduledFutureTime = 0;
    }
...
//					if (DEBUG) LOG.info(System.currentTimeMillis() + " schedule");
	    this.scheduledFuture = flushScheduledExecutor.schedule(asyncFlushDelayTask, delay, TimeUnit.SECONDS);
	} else {
//					if (DEBUG) LOG.info(System.currentTimeMillis() + " submit");
	...
	if (DEBUG)
	    LOG.info(System.currentTimeMillis() + " force reschedule " + newDelay + " + cancel old "
		    + scheduledFutureTime);
	if (scheduledFuture != null) {
	    doCancelFutureSchedule();
	}
	...
	synchronized (lock) {
	    if (DEBUG)
		LOG.info(System.currentTimeMillis() + " onAsyncFlushDelay " + infoToString());
	...
	synchronized (lock) {
	    if (DEBUG)
		LOG.info(System.currentTimeMillis() + " onAsyncFlushImmediate " + infoToString());
	...
	try {
	    ...
	    LOG.error(
		    "Should not occur ... Failed to flush bulk event (from async thread pool) ... ignore, no rethrow!",
		    ex);
	}
	if (bufferedEvents != null) {
	...
	if (DEBUG)
	    LOG.info(System.currentTimeMillis() + " doFlush "
		    + flushParamToString(forceSendAll, sendPartiallyFilledBulk) + " " + infoToString());
	synchronized (asyncSenderLock) { // lock sender, to respect events orders
		...
		if (bufferedEvents != null) {
			...
			if (bulksToSend == null)
			    bulksToSend = new ArrayList<List<T>>(1);
		...
		for (List<T> bulkEvents : bulksToSend) {
		    if (DEBUG)
			LOG.info(System.currentTimeMillis() + " => send bulk: " + bulkEvents.size() + " event(s)");
		    try {
			// **** The biggy ****
	...
	if (asyncDisruptorErrorHandler != null) {
	    ...
	    int rescheduleDelay = asyncDisruptorErrorHandler.onSendEventsFailed(bulkEvents, ex, retryPrependBulks,
		    retryAppendBulks);
	    if (!retryPrependBulks.isEmpty() || !retryAppendBulks.isEmpty()) {
		int countRetryPrepend = 0;
		for (List<T> re : retryPrependBulks)
		    countRetryPrepend += re.size();
		int countRetryAppend = 0;
		for (List<T> re : retryAppendBulks)
		    countRetryAppend += re.size();
		int countReInsert = countRetryPrepend + countRetryAppend;
		LOG.warn("Failed to send bulk events ... disruptorErrorHandler => re-insert " + countReInsert
			+ " event(s)" + ((countRetryAppend != 0) ? " ( " + countRetryAppend + " reordered at end)" : "")
			+ ", ex:" + ex.getMessage());
    ...
    protected void onFailedReInsertBulkEventsToSend(int delayReschedule, List<List<T>> retryPrependBulks,
	    List<List<T>> retryAppendBulks) {
	// similar to sendEvents() ... but events are prepended/appended to
	// asyncEventsBulksQueue instead of appended to list bufferedEvents
	synchronized (lock) {
	...
	return "BulkAsyncSender[" + "flushPeriod=" + flushPeriod + infoToString() + "]";
    }
