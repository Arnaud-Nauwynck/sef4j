...
import org.sef4j.core.utils.AsyncUtils;
...
public class PeriodicTask implements Closeable, IStartableSupport {
    ...
    public PeriodicTask(String displayName, Runnable runnableTask, int period, TimeUnit periodTimeUnit,
	...
	synchronized (lock) {
		...
		scheduledFuture = scheduledExecutor.scheduleWithFixedDelay(wrapperRunnableTimerTask, 0, period,
			periodTimeUnit);
	    } else {
	...
	try {
	    ...
	    LOG.warn("Failed to execute periodic task " + displayName
		    + " ... from timer Thread Pool... ignore, no rethrow!", ex);
	}
	synchronized (lock) {
	    nextScheduledFutureTime = System.currentTimeMillis()
		    + TimeUnit.MILLISECONDS.convert(period, periodTimeUnit);
	}
    }
	...
	public Builder withPeriod(int period, TimeUnit periodTimeUnit) {
	    this.period = period;
	    this.periodTimeUnit = periodTimeUnit;
	    return this;
	}
